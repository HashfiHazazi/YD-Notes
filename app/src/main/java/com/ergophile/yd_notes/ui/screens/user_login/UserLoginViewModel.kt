...
class UserLoginViewModel(private val repository: YDNotesRepository) : ViewModel() {
    private val _userLoginState = MutableStateFlow<ResponseState<UserAuth>>(ResponseState.Idle)

    val userLoginState = _userLoginState.asStateFlow()
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), ResponseState.Idle)

    fun postUserLogin(user: JsonObject) {
        viewModelScope.launch {
            _userLoginState.emitAll(
